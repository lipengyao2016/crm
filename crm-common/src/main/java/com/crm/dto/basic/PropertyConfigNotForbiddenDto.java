...
@ApiModel("产品动态配置信息")
public class PropertyConfigNotForbiddenDto extends BaseDto {
	
	@ApiModelProperty(value = "动态配置属性seqId", example = "0")
	private Integer propertyConfigSeqId;
	
	@ApiModelProperty(value = "动态配置类型(表名)")
	private String permissionType="";
	
	@ApiModelProperty(value = "权限Id或其他动态配置关联id(分类id)", example = "1")
	private String permissionId;
	
	@ApiModelProperty(value = "属性名称(在数据库中保存的字段值)", example = "DESINER_HOBBY")
	private String propertyName;
	
	@ApiModelProperty(value = "属性中文描述", example = "设计师爱好")
	...
	@ApiModelProperty(value = "可否被禁用，禁用后及控件只显示，不可操作", example = "N",hidden=true)
	private String ifForbidden;
	
	@ApiModelProperty(value = "是否必须", example = "Y")
